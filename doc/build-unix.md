UNIX BUILD NOTES
====================
Some notes on how to build MultiSys in Unix.

Note
---------------------
Always use absolute paths to configure and compile MultiSys and the dependencies,
...
This will build multisys-qt as well, if the dependencies are met.
...
memory available when compiling MultiSys. On systems with less, gcc can be
 ...
 **For Ubuntu only:** db4.8 packages are available [here](https://launchpad.net/~multisys/+archive/multisys).
...
To build MultiSys without wallet, see [*Disable-wallet mode*](/doc/build-unix.md#disable-wallet-mode)
...
If you want to build multisys-qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a multisys-qt executable will be
built by default.
...
Notes
-----
The release is built with GCC and then "strip multisysd" to strip the debug
...
Security
--------
To help make your MultiSys installation more secure by making certain attacks impossible to
...
* _Non-executable Stack_: If the stack is executable then trivial stack-based buffer overflow exploits are possible if
    vulnerable buffers are found. By default, MultiSys should be built with a non-executable stack
...
When the intention is to run only a P2P node without a wallet, MultiSys may be compiled in
